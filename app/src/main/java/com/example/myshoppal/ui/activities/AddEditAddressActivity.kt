...
class AddEditAddressActivity : BaseActivity() {
    ...
    fun addUpdateAddressSuccess(){
        ...
        val message: String = if (mAddressDetails != null && mAddressDetails!!.id.isNotEmpty()) {
    ...
    private fun validateData(): Boolean {
        return when {
            ...
            TextUtils.isEmpty(et_full_name.text.toString().trim { it <= ' ' }) || et_full_name.length() < 3 -> {
                showErrorSnackBar(
            ...
            TextUtils.isEmpty(et_phone_number.text.toString().trim { it <= ' ' }) || et_phone_number.length()<=8-> {
                showErrorSnackBar(
